...
if($model->save())
    ...
    $this->redirect(array('//ingresoBodega/view','id'=>$model->id_ingreso_bodega));
...
try{
    ...
    '<h4>Error</h4> No es posible actualizar el registro, la nueva cantidad es menor a la existencia actual en bodega');
    $this->redirect(array('update','id'=>$model->id_detalle_compra));
...
// we only allow deletion via POST request
    ...
    if ( ! $this->loadModel($id)->delete() ) throw new CDbException('Some error message.');
...
Yii::app()->user->setFlash(TbHtml::ALERT_COLOR_ERROR,
'<h4>Error</h4> No es posible eliminar el registro, no hay suficiente existencia en bodega');
//$this->refresh();
//throw new CHttpException(500, 'No es posible eliminar el registro');
...
$mPDF1->SetTitle("Reporte - DetalleCompra");
...
$mPDF1->Output('Reporte - DetalleCompra '.date('YmdHis'),'I');  //Nombre del pdf y parámetro para ver pdf o descargarlo directamente.
exit;
}
